class TestTaskTemplates < Minitest::Test
  ...
  def test_add
    ...
    assert_equal "(\w+): (\d+)", t['opts']['pattern']
    assert_equal "{name}: {value}", t['opts']['template']
  ...
  def test_activities_add_delete
    ...
    activity_uuid, task = Activities.tasktemplate_add('type' => "shell",
                                                      'opts' => {
                                                        'cmd' => "echo 'Hello, world!'",
                                                        'interval' => 1,
                                                        'pattern' => "(\w+): (\d+)",
    ...
    assert_equal "shell", Activities[activity_uuid]['opts']['type']
    assert_equal "echo 'Hello, world!'", Activities[activity_uuid]['opts']['opts']['cmd']
    ...
    assert_equal task['uuid'], Activities[activity_uuid]['opts']['uuid']
  end
